...
class TestPythonVersionConsistency:
    ...
    def test_supported_python_versions_in_ci(self):
        ...
        assert "3.13" in python_versions, "Should support Python 3.13"
    ...
    def test_lint_job_python_version_consistency(self):
        ...
        # Should use environment variable referencing latest supported version (3.13.7)
        assert (
            python_version == "${{ env.PYTHON_VERSION }}"
        ), f"Lint job should use PYTHON_VERSION env var, got {python_version}"
    ...
    def test_dockerfile_python_version_consistency(self):
        ...
        assert (
            len(python_images) >= 2
        ), "Dockerfile should have multiple Python base images"

        # All should use the same version
        unique_versions = set(python_images)
        assert (
            len(unique_versions) == 1
        ), f"All Python images should use same version, got {unique_versions}"

        # Should use our standardized version (3.13.7)
        version = python_images[0]
        assert (
            version == "3.13.7"
        ), f"Dockerfile should use Python 3.13.7, got {version}"
    ...
    def test_devcontainer_python_version(self):
        ...
        # Should use Python 3.13
        assert "3.13" in image, f"DevContainer should use Python 3.13, got {image}"
    ...
    def test_pyproject_black_target_versions(self):
        ...
        target_versions = [
            v.strip().strip("'\"") for v in target_versions_str.split(",")
        ]

        # Should support py312 and py313
        assert "py312" in target_versions, "Black should target Python 3.12"
        assert "py313" in target_versions, "Black should target Python 3.13"
    ...
    def test_renovate_configuration_has_python_managers(self):
        ...
        assert (
            gh_actions_manager is not None
        ), "Should have GitHub Actions Python version manager"
        assert ".github/workflows/" in str(
            gh_actions_manager["fileMatch"]
        ), "Should match workflow files"
        ...
        assert (
            dockerfile_manager is not None
        ), "Should have Dockerfile Python version manager"
